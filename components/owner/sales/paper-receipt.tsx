...
export const PaperReceipt: FC<ReceiptProps> = ({
  ...
  const { total: stateLocal, priceWithTax: stateLocalPriceWithTax } = useInvoice();

  const priceWithTax = total ? total * 0.19 : stateLocalPriceWithTax;

  return (
    <div className="mx-auto w-full max-w-xs text-black shadow-lg" id={`invoice-${invoiceNumber}`}>
      <div className="relative border border-white bg-white p-4 font-mono text-sm">
        ...
        <div className="mb-4 flex justify-center">
          <Barcode value={invoiceNumber} width={1.5} height={50} fontSize={12} />
        </div>
          ...
          <tbody>
            {items.map((item, index) => (
              <tr
                ...
                className={cn("border-gray-200 border-b", index % 2 === 0 ? "bg-gray-50" : "")}
              >
                <td className="max-w-4 truncate text-right text-xs">{item.productName}</td>
                <td className="text-right text-xs">{item.quantity}</td>
                <td className="text-right text-xs">{formatPrice(item.price)}</td>
                <td className="text-right text-xs">{formatPrice(item.quantity * item.price)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="mb-4 text-right text-xs">
          <p className="font-bold">Impuesto (19%): {formatPrice(priceWithTax)}</p>
          <p className="font-bold">Total: {formatPrice(total ?? stateLocal)}</p>
        </div>
...
const Spikes = () => (
  <svg
    ...
    height="90"
    viewBox="0 0 1000 90"
    className="absolute bottom-[-29px] left-[-1px] max-h-max min-w-full max-w-[320px]"
  >
    <path
      id="Triangle_1"
